...
func TestLoadAllDiscoversAndSortsManifestEntries(t *testing.T) {
	...
	writeManifest("mods", "claritymod", "claritymod")
	...
	got := []string{entries[0].Category + "/" + entries[0].ID, entries[1].Category + "/" + entries[1].ID, entries[2].Category + "/" + entries[2].ID, entries[3].Category + "/" + entries[3].ID}
	want := []string{"modpacks/alpha", "modpacks/beta", "mods/claritymod", "resourcepacks/zeta"}
	for i := range want {
		if got[i] != want[i] {
