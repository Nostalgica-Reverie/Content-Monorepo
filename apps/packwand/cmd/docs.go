...
func writeProjectsIndex(quiet bool) (int, error) {
	...
	packs, err := manifest.LoadAll(".")
	if err != nil {
		return 0, err
	}
	for _, pack := range packs {
		root := pack.Category
		path := filepath.Join(pack.Dir, "manifest.json")
		m := pack.Manifest
		...
		auto := manifest.ReadAutomation(pack.Dir)
