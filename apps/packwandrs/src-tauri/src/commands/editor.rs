...
use packwand_collab::protocol::{ChangeKind, FsRequest, PROXYABLE_FS_METHODS};
...
#[derive(Debug)]
pub(crate) struct RemoteFsResult {
...
#[derive(Deserialize)]
#[serde(rename_all = "camelCase")]
struct DocumentWriteParameters {
	id: String,
	path: String,
	content: String,
	expected_hash: String,
...
fn parameters<T: DeserializeOwned>(request: &FsRequest) -> CommandResult<T> {
...
fn remote_root(
	...
	if id != pinned_id {
		return Err(SerializableError::new(
			"collab_scope",
			format!("session is scoped to pack {pinned_id:?}"),
		));
	}
	pack_root(workspace, pinned_id)
}
...
/// Dispatches one allowlisted guest filesystem request through the same
/// pack-root and safe-join primitives used by the local command wrappers.
pub(crate) fn remote_fs_dispatch(
	workspace: &std::path::Path,
	pinned_id: &str,
	request: &FsRequest,
) -> CommandResult<RemoteFsResult> {
	if !PROXYABLE_FS_METHODS.contains(&request.method.as_str()) {
		return Err(SerializableError::new(
			"collab_protocol",
			format!("filesystem method {:?} is not proxyable", request.method),
		));
	}
	...
	let value = match request.method.as_str() {
		"editor_document_read" => {
			let input: PathParameters = parameters(request)?;
			validate_remote_path(&input.path)?;
			let root = remote_root(workspace, pinned_id, &input.id)?;
			serde_json::to_value(document_at(&safe_join(&root, &input.path)?)?)?
		}
		"editor_document_write" => {
			let input: DocumentWriteParameters = parameters(request)?;
			validate_remote_path(&input.path)?;
			let root = remote_root(workspace, pinned_id, &input.id)?;
			let target = safe_join(&root, &input.path)?;
			let current = document_at(&target)?;
			if current.hash != input.expected_hash {
				return Err(SerializableError::new(
					"file_conflict",
					format!("{} changed outside Packwand", target.display()),
				));
			}
			crate::fsutil::atomic_write(&target, input.content.as_bytes())?;
			changes.push((input.path, ChangeKind::Modified));
			serde_json::to_value(document_at(&target)?)?
		}
		"editor_search" => {
			let input: SearchParameters = parameters(request)?;
			let root = remote_root(workspace, pinned_id, &input.id)?;
			if input.query.is_empty() {
				serde_json::to_value(Vec::<SearchMatch>::new())?
			} else {
				serde_json::to_value(search_inner(
					&root,
					input.query,
		...
		"editor_fs_stat" => {
			...
			validate_remote_path(&input.path)?;
			let root = remote_root(workspace, pinned_id, &input.id)?;
			let metadata = fs::metadata(safe_join(&root, &input.path)?)
				.map_err(|error| io_error("stat", error))?;
			serde_json::to_value(EditorFileStat {
				file_type: file_type(&metadata),
				size: metadata.len(),
				ctime: timestamp(metadata.created()),
				mtime: timestamp(metadata.modified()),
		...
		"editor_fs_read_dir" => {
			...
			validate_remote_path(&input.path)?;
			let root = remote_root(workspace, pinned_id, &input.id)?;
			let mut entries = fs::read_dir(safe_join(&root, &input.path)?)
				.map_err(|error| io_error("read directory", error))?
				.map(|entry| {
					let entry = entry.map_err(|error| io_error("read directory", error))?;
					let metadata = entry
		...
		"editor_fs_read_file" => {
			...
			let root = remote_root(workspace, pinned_id, &input.id)?;
			serde_json::to_value(
				fs::read(safe_join(&root, &input.path)?)
					.map_err(|error| io_error("read file", error))?,
			)?
		}
		"editor_fs_write_file" => {
			let input: WriteFileParameters = parameters(request)?;
			validate_remote_path(&input.path)?;
			let root = remote_root(workspace, pinned_id, &input.id)?;
			let target = safe_join(&root, &input.path)?;
			let exists = target.exists();
			...
			crate::fsutil::atomic_write(&target, &input.content)?;
			changes.push((
				input.path,
				if exists {
					ChangeKind::Modified
		...
		"editor_fs_create_dir" => {
			...
			validate_remote_path(&input.path)?;
			let root = remote_root(workspace, pinned_id, &input.id)?;
			fs::create_dir(safe_join(&root, &input.path)?)
				.map_err(|error| io_error("create directory", error))?;
			changes.push((input.path, ChangeKind::Created));
			serde_json::Value::Null
		}
		"editor_fs_delete" => {
			let input: DeleteParameters = parameters(request)?;
			...
			validate_remote_path(&input.path)?;
			let root = remote_root(workspace, pinned_id, &input.id)?;
			remove_path(&safe_join(&root, &input.path)?, input.recursive)?;
			changes.push((input.path, ChangeKind::Deleted));
			serde_json::Value::Null
		}
		"editor_fs_rename" => {
			let input: RenameParameters = parameters(request)?;
			...
			validate_remote_path(&input.from)?;
			validate_remote_path(&input.to)?;
			let root = remote_root(workspace, pinned_id, &input.id)?;
			let source = safe_join(&root, &input.from)?;
			let target = safe_join(&root, &input.to)?;
			if target.exists() {
				if !input.overwrite {
					return Err(SerializableError::new(
						"already_exists",
			...
			fs::rename(&source, &target).map_err(|error| io_error("rename", error))?;
			changes.push((input.from, ChangeKind::Deleted));
			changes.push((input.to, ChangeKind::Created));
			serde_json::Value::Null
		}
		_ => unreachable!("the allowlist and dispatcher must stay exhaustive"),
	};
	Ok(RemoteFsResult { value, changes })
}
