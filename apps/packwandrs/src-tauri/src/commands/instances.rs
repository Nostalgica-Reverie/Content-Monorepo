...
fn install_instance(
    ...
    let result = packwand_build::test_with_installer(&pack_dir, installer_jar, &game_dir)
        .map_err(|error| domain_error("installer", error))
        .and_then(|_| restore_disabled(&game_dir, &disabled));
    match result {
...
pub async fn instances_install(
    ...
    repo.get(&id)
        .map_err(|error| domain_error("instance", error))?;
    let installer = find_installer_jar(&app);
...
mod tests {
    ...
    fn disabled_content_wins_after_reinstall() {
        ...
        restore_disabled(root.path(), &[disabled.clone()]).unwrap();
