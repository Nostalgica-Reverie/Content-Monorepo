...
use packwand_providers::{
	BrowsePage, BrowseQuery, CreatorProfile, CurseForgeClient, ForgejoClient, GitHubClient,
	GitLabClient, ModrinthClient, ProviderBrowser, ProviderKind, ProviderResolver, ResolveRequest,
	ResolvedProject, UreqTransport, configured_api_key,
};

/// The CurseForge key to use for a request, in precedence order.
///
/// 1. A key the caller passed explicitly.
/// 2. The key the user connected in Settings, from the OS keychain.
/// 3. `configured_api_key` — the `PACKWAND_CURSEFORGE_API_KEY` /
///    `CURSEFORGE_API_KEY` / `CF_API_KEY` environment variables, falling back
///    to the bundled key.
///
/// The keychain step is what stops the GUI depending on the shell environment
/// it happened to be launched from. Before it existed the frontend passed
/// `None`, which reached CurseForge as an empty `X-API-Key` header and came
/// back 403 — every CurseForge call from the GUI failed while the identical
/// call from the CLI worked.
fn curseforge_key(token: Option<String>) -> String {
	token
		.map(|value| value.trim().to_owned())
		.filter(|value| !value.is_empty())
		.or_else(crate::commands::accounts::stored_curseforge_key)
		.unwrap_or_else(configured_api_key)
}

/// The Modrinth token to use for a request, if any.
///
/// Modrinth's public endpoints work unauthenticated, so `None` is a normal
/// state rather than a failure — a linked account raises rate limits and makes
/// the user's own draft projects visible.
fn modrinth_token(token: Option<String>) -> Option<String> {
	token
		.map(|value| value.trim().to_owned())
		.filter(|value| !value.is_empty())
		.or_else(crate::commands::accounts::stored_modrinth_token)
}
...
fn resolve(
	...
	// Resolved before the `unwrap_or_default` below, which the repository
	// providers rely on to mean "anonymous".
	let modrinth = modrinth_token(token.clone());
	let token = token.unwrap_or_default();
	let transport = UreqTransport::new();
	Ok(match provider {
		ProviderKind::Modrinth => ModrinthClient::new(transport)
			.with_token(modrinth)
			.resolve(request)?,
...
pub async fn providers_browse(
	...
	tokio::task::spawn_blocking(move || {
		...
		let transport = UreqTransport::new();
		match provider {
			ProviderKind::Modrinth => Ok(ModrinthClient::new(transport)
				.with_token(modrinth_token(token))
				.search(&query)?),
			ProviderKind::CurseForge => {
...
///
/// Only the catalogue providers have creators to look up. What comes back
/// differs by provider and `CreatorProfile::partial` says which case this is:
/// Modrinth returns a real profile, CurseForge a name and whatever projects a
/// search can attribute to it, because their API has no user resource.
#[tauri::command]
pub async fn providers_creator(
	provider: ProviderKind,
	handle: String,
	token: Option<String>,
) -> CommandResult<CreatorProfile> {
	tokio::task::spawn_blocking(move || {
		let transport = UreqTransport::new();
		match provider {
			ProviderKind::Modrinth => Ok(ModrinthClient::new(transport)
				...
				.creator(&handle)?),
			ProviderKind::CurseForge => {
				Ok(CurseForgeClient::new(transport, curseforge_key(token)).creator(&handle)?)
			}
			other => Err(SerializableError::new(
				"not_browsable",
				format!("{} has no creator profiles", other.name()),
			)),
		}
	})
	.await
