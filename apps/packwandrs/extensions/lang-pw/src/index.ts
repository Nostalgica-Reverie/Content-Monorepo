	...
	commands: [
		{
			...
			async run(context) {
				...
				const files = new Map(
					snapshot.files.map((file) => [`${file.namespace}:${file.locale}`, file.path]),
				)
				...
				context.output(
					`LangPW: ${snapshot.files.length} language file(s), ${snapshot.gaps.length} missing key(s).`,
					issues.length ? 'error' : 'success',
				)
				context.notify(
					'LangPW',
					issues.length
						? `${issues.length} missing translation key(s).`
