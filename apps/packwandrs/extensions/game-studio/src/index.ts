  ...
  commands: [
    {
      ...
      async run(context) {
        ...
        context.output(`GamePW Studio: ${count} issue(s) across ${report.checked} file(s).`, count ? 'error' : 'success')
        context.notify('GamePW Studio', count ? `${count} issue(s) found.` : 'Game content is valid.', count ? 'danger' : 'success')
      },
    },
    {
      id: 'recipes',
      ...
      async run(context) {
        ...
        context.output(`GamePW Studio: indexed ${recipes.length} recipe(s) in ${namespaces} namespace(s).`, 'success')
    ...
    },
    {
      ...
      async run(context) {
        ...
        const drifted = reports.filter(report =>
        ...
        context.output(`GamePW Studio: ${drifted.length} of ${reports.length} variant(s) drifted.`, drifted.length ? 'error' : 'success')
        context.notify('Variant parity', drifted.length ? `${drifted.length} variant(s) drifted.` : 'All variants agree.', drifted.length ? 'danger' : 'success')
      },
    },
  ],
  views: [
    {
      ...
      async rows(context) {
        ...
        return registries
          ...
          .flatMap(registry => registry.entries.map(entry => {
