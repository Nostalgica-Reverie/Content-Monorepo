...
export default definePackwandExtension({
  commands: [{
    ...
    async run(context) {
      ...
      const issues = report.issues.filter(issue => issue.path.replaceAll('\\', '/').includes('/worldgen/'))
      context.publishProblems('WorldgenPW', issues)
      const kinds = new Set(assets.map(asset => asset.kind)).size
      context.output(`WorldgenPW: ${assets.length} definition(s) across ${kinds} registry type(s); ${issues.length} issue(s).`, issues.length ? 'error' : 'success')
      context.notify('WorldgenPW', issues.length ? `${issues.length} worldgen issue(s).` : `${assets.length} definition(s) indexed.`, issues.length ? 'danger' : 'success')
    },
  }],
  views: [{
    id: 'worldgen',
