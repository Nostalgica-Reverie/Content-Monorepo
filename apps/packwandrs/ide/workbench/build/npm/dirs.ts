...
// Packwand intentionally removes non-pack authoring extensions. Keep the
// upstream installer list declarative, but do not invoke npm in pruned roots.
for (let index = dirs.length - 1; index >= 0; index--) {
	const directory = dirs[index];
	if (directory && !existsSync(`${import.meta.dirname}/../../${directory}`)) {
		dirs.splice(index, 1);
	}
}
