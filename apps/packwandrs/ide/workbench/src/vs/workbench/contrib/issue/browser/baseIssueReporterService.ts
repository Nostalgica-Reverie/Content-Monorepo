...
export class BaseIssueReporterService extends Disposable {
	...
	public searchVSCodeIssues(title: string, issueDescription?: string): void {
		if (title) {
			this.searchGitHub('Lasting-Legacy/Lasting-Legacy-Monorepo', title);
		} else {
			this.clearSearchResults();
		}
	}
	...
	public setSourceOptions(): void {
		...
		sourceSelect.append(this.makeOption(IssueSource.VSCode, localize('vscode', "Packwand IDE"), false));
		sourceSelect.append(this.makeOption(IssueSource.Extension, localize('extension', "A Packwand IDE extension"), false));
	...
	public async renderBlocks(): Promise<void> {
		...
		if (selectedExtension && this.nonGitHubIssueUrl) {
			...
			reset(descriptionTitle, localize('handlesIssuesElsewhere', "This extension handles issues outside of Packwand IDE"));
