...
export class ChatStatusBarEntry extends Disposable implements IWorkbenchContribution {
	...
	private update(): void {
		// Packwand: `sentiment.hidden` is derived from ChatEntitlementContext's
		// internal state, which is only recomputed once an async `updateContext()`
		// pass completes (and can be indefinitely deferred by a `suspend()` whose
		// matching `resume()` never fires, e.g. while waiting on a marketplace/
		// entitlement round trip that never resolves without a real Copilot
		// extension installed). Nearly every other chat-related command/menu in
		// this fork is instead gated directly on the live `config.chat.disableAIFeatures`
		// context key, which has no such lag. Check the raw setting here too so this
		// status bar item can't outlive the setting that's supposed to hide it.
		const aiFeaturesDisabled = this.configurationService.getValue<boolean>(ChatConfiguration.AIDisabled) === true;
		const sentiment = this.chatEntitlementService.sentiment;
		if (!aiFeaturesDisabled && !sentiment.hidden) {
	...
	private registerListeners(): void {
		...
		this._register(this.configurationService.onDidChangeConfiguration(e => {
			if (e.affectsConfiguration(product.defaultChatAgent?.completionsEnablementSetting) || e.affectsConfiguration(ChatConfiguration.TitleBarSignInEnabled) || e.affectsConfiguration(ChatConfiguration.AIDisabled)) {
				this.update();
			}
		}));
	}
