...
/// The credentials an upload needs, resolved by the caller.
///
/// Exists because the two callers get their secrets from different places and
/// neither should have to pretend to be the other. The CLI and Forgejo Actions
/// read the environment, which is the right interface there; the desktop app
/// reads the OS keychain, and mutating its own process environment to suit a
/// library would be both racy and unnecessary.
///
/// [`PublishCredentials::from_env`] preserves the original behaviour exactly,
/// so nothing in CI changes.
...
impl PublishCredentials {
	/// Reads `MODRINTH_TOKEN`, `CURSEFORGE_TOKEN` and `CURSEFORGE_API_KEY`.
	pub fn from_env() -> Self {
		fn variable(name: &str) -> Option<String> {
			std::env::var(name).ok().filter(|value| !value.is_empty())
		}
		Self {
			modrinth_token: variable("MODRINTH_TOKEN"),
			curseforge_token: variable("CURSEFORGE_TOKEN"),
			curseforge_api_key: variable("CURSEFORGE_API_KEY"),
		}
	}

	/// Fills any credential this value is missing from the environment.
	pub fn or_env(mut self) -> Self {
		let environment = Self::from_env();
		self.modrinth_token = self.modrinth_token.or(environment.modrinth_token);
		self.curseforge_token = self.curseforge_token.or(environment.curseforge_token);
		self.curseforge_api_key = self.curseforge_api_key.or(environment.curseforge_api_key);
		self
	}
}

/// Uploads using credentials from the environment.
...
pub fn upload_publish_target_with(
	manifest_path: impl AsRef<Path>,
	variant: Option<&str>,
	live: bool,
	changelog_file: Option<&Path>,
	...
	for artifact in target.artifacts.iter().filter(|artifact| artifact.exists) {
		...
		if artifact.platform == "modrinth" {
			upload_modrinth(credentials, &target, project, artifact, &changelog)?;
		} else {
			upload_curseforge(credentials, &target, project, artifact, &changelog)?;
		}
		report.uploaded.push(artifact.platform.clone());
	}
...
fn upload_modrinth(
	...
	let token = credentials
		.modrinth_token
		.clone()
		.ok_or("no Modrinth token: connect Modrinth in Settings, or set MODRINTH_TOKEN")?;
...
fn upload_curseforge(
	...
	let token = credentials
		.curseforge_token
		.clone()
		.ok_or("no CurseForge upload token: add one in Settings, or set CURSEFORGE_TOKEN")?;
...
fn curseforge_version_exists(
	credentials: &PublishCredentials,
	project: &str,
	display_name: &str,
) -> bool {
	let Some(key) = credentials.curseforge_api_key.clone() else {
		return false;
	};
