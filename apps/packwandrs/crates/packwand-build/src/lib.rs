...
pub use installer::{
	InstallerTestReport, ManualDownload, install_with_native_installer, manual_pending,
	provide_manual_download, test_with_installer,
};
pub use packeater::{
	PACKEATER_MARKER, archive_content_directory, discover_packeater_markers, run_packeater,
};
pub use publish::{
	PublishArtifact, PublishCredentials, PublishMatrixEntry, PublishTarget, PublishUploadReport,
	build_publish_target, list_publish_targets, resolve_publish_target, upload_publish_target,
	upload_publish_target_with, verify_publish_target,
};
...
mod tests {
	...
	"hash-format": "sha512",
	"files": [
		{ "file": "config/example.json", "hash": "a" },
		{ "file": "mods/a.pw.json", "hash": "b", "metafile": true }
	]
}
"#,
		)
	...
	"download": {
		...
		"hash": "def",
		"size": 3,
		"extra-hashes": { "sha1": "abc" }
	},
	"update": { "curseforge": { "project-id": 10, "file-id": 20 } }
}
"#,
		)
		.unwrap();
  ...
  "overrides": "overrides"
		...
		let metadata: packwand_pack::Mod = serde_json::from_str(
	...
	fn stages_more_files_than_fit_in_one_batch() {
		...
		for slot in 0..count {
			...
			files.push(serde_json::json!({
				"file": format!("config/file{slot}.json"),
				"hash": "a",
			}));
		}
		let index = serde_json::json!({ "hash-format": "sha512", "files": files });
		std::fs::write(
			directory.path().join("index.json"),
