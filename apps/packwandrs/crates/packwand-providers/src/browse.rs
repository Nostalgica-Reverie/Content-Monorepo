...
/// Everything a provider can say about the person or team behind a project.
///
/// How complete this is differs sharply by provider, and the difference is
/// theirs rather than ours:
///
///  - **Modrinth** has a real user resource — avatar, bio, join date, and a
///    listing of the user's projects.
///  - **CurseForge** has no standalone user endpoint at all. An author is a
///    name and a numeric id embedded in each mod payload, so everything except
///    [`CreatorProfile::name`] and the projects we can find by searching that
///    name will be empty. [`CreatorProfile::partial`] says so explicitly, so
///    the UI can tell the user rather than rendering a profile that looks
///    broken.
#[derive(Debug, Clone, Default, PartialEq, Eq, Serialize, Deserialize)]
#[serde(rename_all = "camelCase")]
pub struct CreatorProfile {
	/// Provider-native handle, which is what [`ProviderBrowser::creator`] takes.
	pub handle: String,
	pub name: String,
	#[serde(default, skip_serializing_if = "Option::is_none")]
	...
	#[serde(default)]
	pub bio: String,
	/// ISO-8601, as the provider reports it. Not parsed: the only thing the UI
	/// does with it is show it, and a parse here would be a second place for a
	/// provider's format change to break.
	#[serde(default, skip_serializing_if = "Option::is_none")]
	...
	#[serde(default)]
	pub projects: Vec<BrowseProject>,
	/// True when the provider cannot supply a real profile and this is a
	/// best-effort reconstruction from project metadata.
	#[serde(default)]
	pub partial: bool,
}
...
pub trait ProviderBrowser {
	...
	/// The person or team behind a project, by provider-native handle.
	fn creator(&self, handle: &str) -> Result<CreatorProfile, ProviderError>;
}
