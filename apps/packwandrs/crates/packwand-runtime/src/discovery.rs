//! Finding the Java installations already on the machine.
//!
//! Discovery is split from the host environment so tests can run against
//! synthetic JDK layouts: [`DiscoveryConfig::from_host`] captures the real
//! environment, while the pure functions accept explicit paths.
//!
//! Nothing here starts a process. Reading a `release` file is cheap enough to
//! do for every candidate on the machine; identifying a JVM by running it is
//! not, and lives in [`crate::probe`] behind a cache.
