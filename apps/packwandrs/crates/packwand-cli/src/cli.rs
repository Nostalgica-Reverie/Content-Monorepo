...
fn diagnostics() -> Vec<Command> {
	vec![
		...
		command("registry", "Build content registries")
			...
			.arg(flag("json", "Output JSON")),
		command("test", "Serve a pack and run native installer validation")
			.arg(positional("pack-subdir", "Pack subdir").required(true)),
...
fn other() -> Vec<Command> {
	vec![
		command("account", "Manage the signed-in ATProto identity")
			.subcommand(command("login", "Sign in with an ATProto handle or DID"))
			.subcommand(
				command("whoami", "Show the signed-in ATProto identity")
					.arg(flag("json", "Output JSON")),
			)
			.subcommand(command(
				"logout",
				"Sign out and clear the local OAuth session",
			)),
		...
		command("change", "Manage stacked Jujutsu changes")
			.subcommand(command(
				"enable",
				"Enable Jujutsu alongside the existing Git repository",
			))
			.subcommand(
				command("new", "Create a new working-copy change")
					.arg(option("parent", "Parent change ID or revision"))
					.arg(flag("json", "Output JSON")),
			)
			.subcommand(
				command("describe", "Replace a change description")
					.arg(positional("change-id", "Change ID").required(true))
					.arg(positional("message", "New description").required(true)),
			)
			.subcommand(
				command("squash", "Squash a change into its first parent")
					.arg(positional("change-id", "Change ID").required(true))
					.arg(flag(
						"into-parent",
						"Confirm squashing into the first parent",
					)),
			)
			.subcommand(
				command("log", "Show the local stacked-change graph")
					.arg(flag("json", "Output JSON")),
			),
		command("diff", "Show mod changes between two git refs")
			...
			.arg(flag("json", "Output JSON")),
		command("friends", "Discover friends and send collaboration invites")
			.subcommand(
				command("list", "List mutual follows and Packwand contacts")
					.arg(flag("json", "Output JSON")),
			)
			.subcommand(
				command(
					"invite",
					"Publish a collaboration invite for a friend's DID",
				)
				.arg(positional("did", "Recipient ATProto DID").required(true))
				.arg(positional("invite", "Collaboration pw:// invite").required(true))
				.arg(
					option("expires-in", "Invite lifetime in minutes")
						.default_value("60")
						.value_parser(clap::value_parser!(u64).range(1..=10_080)),
				)
				.arg(flag("json", "Output JSON")),
			),
		...
		command(
			"share",
			"Publish Packwand content to your ATProto repository",
		)
		.subcommand(
			command("pack", "Share a manifest summary and source repository")
				.arg(positional("dir", "Pack directory").default_value("."))
				.arg(option("tangled-repo", "Linked Tangled repository AT URI"))
				.arg(option("git-remote", "Plain Git remote; defaults to origin"))
				.arg(flag("json", "Output JSON")),
		)
		.subcommand(
			command("snippet", "Share a UTF-8 text file")
				.arg(positional("file", "Text file").required(true))
				.arg(option("language", "Snippet language identifier"))
				.arg(flag("json", "Output JSON")),
		)
		.subcommand(
			command("image", "Upload and share an image")
				.arg(positional("file", "Image file").required(true))
				.arg(option("caption", "Image caption"))
				.arg(option("mime", "Image MIME type"))
				.arg(flag("json", "Output JSON")),
		),
		command(
			"script",
		...
		command("somnus", "Run Tangled-compatible workflows locally")
			.subcommand(
				command(
					"run",
					"Run one workflow or every locally triggered workflow",
				)
				.arg(positional("workflow", "Workflow YAML file")),
			)
			.subcommand(
				command("list", "List workflows and local trigger state")
					.arg(flag("json", "Output JSON")),
			)
			.subcommand(
				command("status", "Show the last local workflow run")
					.arg(flag("json", "Output JSON")),
			),
	]
}
