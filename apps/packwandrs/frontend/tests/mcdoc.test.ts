...
describe('simplifyType', () => {
  test('follows a reference to its definition', () => {
    ...
    const simplified = simplifyType({ kind: 'reference', path: '::test::Name' }, rootPath(''), source)
    expect(simplified.kind).toBe('string')
  })

  test('degrades an unresolvable reference to any rather than throwing', () => {
    const simplified = simplifyType({ kind: 'reference', path: '::missing' }, rootPath(''), emptySchemaSource)
    expect(simplified.kind).toBe('any')
  })

  test('stops on a reference cycle instead of hanging', () => {
    const source = createMemorySchemaSource({
      references: { '::a': { kind: 'reference', path: '::b' }, '::b': { kind: 'reference', path: '::a' } },
    })
...
describe('structFields', () => {
  test('inlines spread fields', () => {
    ...
    const struct = {
      kind: 'struct',
      fields: [stringField('own'), { kind: 'spread', type: { kind: 'reference', path: '::shared' } }],
    } as const
  ...
  test('a later field with the same key overrides an earlier one', () => {
    const struct = {
      kind: 'struct',
      fields: [stringField('id'), { kind: 'pair', key: 'id', type: { kind: 'boolean' }, optional: true }],
    } as const
...
describe('defaultValue', () => {
  ...
  test('terminates on a self-referential struct', () => {
    const source = createMemorySchemaSource({
      references: {
        '::node': { kind: 'struct', fields: [{ kind: 'pair', key: 'child', type: { kind: 'reference', path: '::node' } }] },
      },
    })
...
describe('selectUnionMember', () => {
  ...
  test('a literal tag outweighs a structurally similar member', () => {
    const tagged: McdocType[] = [
      { kind: 'struct', fields: [{ kind: 'pair', key: 'type', type: { kind: 'literal', value: { kind: 'string', value: 'a' } } }] },
      { kind: 'struct', fields: [{ kind: 'pair', key: 'type', type: { kind: 'literal', value: { kind: 'string', value: 'b' } } }] },
    ]
...
describe('attributes', () => {
  test('reads a registry off an id attribute', () => {
    const type: McdocType = {
      kind: 'string',
      attributes: [{ name: 'id', value: { kind: 'literal', value: { kind: 'string', value: 'minecraft:block' } } }],
    }
  ...
  test('survives a reference being followed', () => {
    ...
    const type: McdocType = {
      kind: 'reference',
      path: '::id',
      attributes: [{ name: 'id', value: { kind: 'literal', value: { kind: 'string', value: 'minecraft:item' } } }],
    }
...
describe('typeLabel', () => {
  test('names union members readably', () => {
    expect(typeLabel({ kind: 'literal', value: { kind: 'string', value: 'set_count' } })).toBe('set_count')
