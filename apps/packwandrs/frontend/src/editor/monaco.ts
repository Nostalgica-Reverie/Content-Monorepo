...
export function registerPackwandLanguages() {
  ...
  monaco.languages.setMonarchTokensProvider('toml', {
    tokenizer: {
      root: [
        [/#.*$/, 'comment'], [/\[\[?.*?\]\]?/, 'type'], [/^[\w.-]+(?=\s*=)/, 'key'],
        [/"(?:[^"\\]|\\.)*"/, 'string'], [/'[^']*'/, 'string'], [/\b(?:true|false)\b/, 'keyword'],
        [/[+-]?(?:\d+\.?\d*|\.\d+)/, 'number'],
      ],
    },
  })
  ...
  monaco.languages.setMonarchTokensProvider('mcfunction', {
    tokenizer: {
      root: [
        [/#.*$/, 'comment'], [/^\s*\/?[a-z_][\w:]*/, 'keyword'], [/@[pares](?:\[[^\]]*\])?/, 'type'],
        [/\b(?:true|false)\b/, 'keyword'], [/[~-]?\d+(?:\.\d+)?/, 'number'], [/"(?:[^"\\]|\\.)*"/, 'string'],
...
export function languageForPath(path: string) {
  ...
  return ({
    json: 'json', json5: 'json', toml: 'toml', mcfunction: 'mcfunction', js: 'javascript', mjs: 'javascript',
    cjs: 'javascript', ts: 'typescript', mts: 'typescript', css: 'css', scss: 'scss', html: 'html', md: 'markdown',
    xml: 'xml', yml: 'yaml', yaml: 'yaml', properties: 'ini', txt: 'plaintext', log: 'plaintext',
...
export function installMonacoTheme(theme: ResolvedTheme) {
  ...
  monaco.editor.defineTheme(name, {
    base: theme.appearance === 'light' ? 'vs' : theme.appearance === 'high-contrast' ? highContrastBase : 'vs-dark',
    inherit: true,
