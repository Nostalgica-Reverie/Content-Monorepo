...
export function validateTheme(value: unknown): ThemeValidation {
	...
	if (typeof theme.id !== 'string' || !validateThemeId(theme.id))
		errors.push('id must be a lowercase builtin.* or user.* slug.')
	if (typeof theme.name !== 'string' || !theme.name.trim()) errors.push('name is required.')
	if (!['light', 'dark', 'high-contrast'].includes(String(theme.appearance)))
		errors.push('appearance must be light, dark, or high-contrast.')
	if (
		theme.extends !== undefined &&
		(typeof theme.extends !== 'string' || !builtinThemeMap.has(theme.extends))
	)
		errors.push('extends must reference a bundled theme.')
	if (!theme.colors || typeof theme.colors !== 'object' || Array.isArray(theme.colors))
		errors.push('colors must be an object.')
	else {
		for (const [key, color] of Object.entries(theme.colors)) {
			if (!themeTokenNames.includes(key as ThemeTokenName))
				errors.push(`Unknown application color token: ${key}.`)
			if (typeof color !== 'string' || !validateHexColour(color))
				errors.push(`${key} must be a #RRGGBB or #RRGGBBAA color.`)
		}
	}
	for (const [key, color] of Object.entries(theme.editor?.colors ?? {})) {
		if (!key || typeof color !== 'string' || !validateHexColour(color))
			errors.push(`Invalid editor color: ${key}.`)
	}
	for (const rule of theme.editor?.rules ?? []) {
		if (!rule || typeof rule.token !== 'string' || !rule.token.trim())
			errors.push('Every editor rule needs a token.')
		if (rule.foreground && !validateHexColour(rule.foreground))
			errors.push(`Invalid token foreground for ${rule.token}.`)
		if (rule.background && !validateHexColour(rule.background))
			errors.push(`Invalid token background for ${rule.token}.`)
		if (
			rule.fontStyle !== undefined &&
			!['', 'bold', 'italic', 'underline', 'strikethrough'].includes(rule.fontStyle)
		)
			errors.push(`Invalid token fontStyle for ${rule.token}.`)
	}
	if (theme.id?.startsWith('builtin.'))
		warnings.push('Bundled theme IDs are reserved; imported themes will be copied to a user.* ID.')
	if (!errors.length) {
		...
		for (const { foreground, background, label, minimum } of contrastRequirements()) {
			...
			if (ratio === null) warnings.push(`${label} contrast could not be measured.`)
				...
				warnings.push(`${label} contrast is ${ratio.toFixed(2)}:1; target at least ${minimum}:1.`)
		}
	}
...
export function resolveTheme(theme: PackwandTheme): ResolvedTheme {
	...
	return {
		...theme,
		colors: { ...(baseResolved?.colors ?? fallback.colors), ...theme.colors } as Record<
			ThemeTokenName,
			string
		>,
		editor: {
			colors: {
				...(baseResolved?.editor.colors ?? fallback.editor?.colors),
				...(theme.editor?.colors ?? {}),
			},
			rules: [
				...(baseResolved?.editor.rules ?? fallback.editor?.rules ?? []),
				...(theme.editor?.rules ?? []),
			],
		},
	}
...
export function portableTheme(theme: ResolvedTheme, id: string, name: string): PackwandTheme {
	return {
		...
		editor: {
			...
			rules: theme.editor.rules.map((rule) => ({ ...rule })),
		},
	}
}
