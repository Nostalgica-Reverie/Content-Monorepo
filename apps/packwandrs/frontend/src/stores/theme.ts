...
function slug(value: string) {
  return value.toLowerCase().trim().replace(/[^a-z0-9]+/g, '-').replace(/^-|-$/g, '') || 'theme'
}
...
export const useThemeStore = defineStore('theme', () => {
  ...
  const currentTheme = computed(() => themes.value.find(theme => theme.id === currentId.value) ?? builtinThemes[0])
  ...
  async function saveCustom(theme: PackwandTheme) {
    ...
    customThemes.value = [...customThemes.value.filter(item => item.id !== saved.id), saved]
  ...
  function duplicate(source: PackwandTheme = currentTheme.value) {
    ...
    let suffix = 2
    while (themes.value.some(theme => theme.id === id)) id = `user.${slug(source.name)}-${suffix++}`
  ...
  return {
    themes, currentId, currentTheme, resolved, loading, diagnostics,
    initialize, setTheme, applySetting, preview, cancelPreview, saveCustom, removeCustom, duplicate, importText, exportText,
  }
})
