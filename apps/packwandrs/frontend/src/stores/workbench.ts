...
import type { SessionInfo } from '@/helpers/invoke/collab'
...
export const useWorkbenchStore = defineStore('workbench', () => {
	...
	const selectedProject = computed(
		() =>
			projects.value.find((project) => project.manifest.id === selectedProjectId.value) ??
			projects.value[0] ??
			null,
	)
	...
	const selectedPack = computed(
		() =>
			projectPacks.value.find((pack) => pack.id === selectedPackId.value) ??
			projectPacks.value[0] ??
			null,
	)
	...
	function requestDiff(diff: GitDiffDocument) {
		requestedDiff.value = diff
	}
	function setRemotePack(session: SessionInfo) {
		projects.value = []
		packs.value = [
			{
				id: session.packId,
				name: session.packName,
				path: `host/${session.packId}`,
				packFormat: 'remote',
				version: '',
		...
		selectedPackId.value = session.packId
	}
	async function refresh() {
		...
		try {
			...
			if (selectedProjectId.value)
				localStorage.setItem('packwand:selected-project', selectedProjectId.value)
			if (selectedPackId.value) localStorage.setItem('packwand:selected-pack', selectedPackId.value)
		} catch (caught) {
