...
import type { DiagnosticIssue, JobRecord } from '@/helpers/types'
import { clampSize, closeTabIn, openTabIn } from '@/core/packwand'

export type DockTab = 'problems' | 'output' | 'logs' | 'terminal'
export type SidebarMode = 'explorer' | 'source-control' | 'collab' | 'extensions'
...
export const useShellStore = defineStore('shell', () => {
	...
	const sidebarMode = ref<SidebarMode>(
		(localStorage.getItem('packwand:sidebar-mode') as SidebarMode) || 'explorer',
	)
	...
	const hostJobs = ref<JobRecord[]>([])
	let outputSeq = 0

	const errorCount = computed(
		() => problems.value.filter((issue) => issue.severity === 'error').length,
	)
	const warningCount = computed(
		() => problems.value.filter((issue) => issue.severity === 'warning').length,
	)
	...
	function appendOutput(text: string, tone: OutputTone = 'info', fromHost = false) {
		const time = new Date().toLocaleTimeString(undefined, { hour12: false })
		for (const line of text.split('\n')) {
			output.value.push({ id: ++outputSeq, time, text: line, tone, fromHost })
		}
		if (output.value.length > OUTPUT_LIMIT)
			output.value.splice(0, output.value.length - OUTPUT_LIMIT)
	}
	...
	function applyHostJobEvent(event: string, payload: Record<string, unknown>) {
		if (event === 'started') {
			const job = payload as unknown as JobRecord
			hostJobs.value = [...hostJobs.value.filter((candidate) => candidate.id !== job.id), job]
			return
		}
		const id = String(payload.id ?? '')
		const job = hostJobs.value.find((candidate) => candidate.id === id)
		if (!job) return
		if (event === 'log') job.logs.push(String(payload.line ?? ''))
		else if (event === 'progress') {
			job.fraction = Number(payload.fraction ?? job.fraction)
			job.message = payload.message === null ? null : String(payload.message ?? job.message ?? '')
		} else if (event === 'done' || event === 'failed') {
			job.status = event
			job.error = (payload.error as JobRecord['error']) ?? null
		}
	}
