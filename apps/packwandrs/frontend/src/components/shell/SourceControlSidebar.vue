...
import ChangesPanel from '@/components/shell/ChangesPanel.vue'
...
const canWrite = computed(() => !collab.isGuest || collab.allowGitWrite)
...
<template>
	<div class="source-control">
		...
		<div v-if="status" class="source-control__branch">
			...
			<span v-if="status.ahead || status.behind" class="tree-row__meta"
		...
		<form class="source-control__commit" @submit.prevent="commit">
			...
			/>
			<button type="submit" :disabled="loading || !canWrite || !message.trim() || !staged.length">
				Commit
			</button>
		</form>
		...
		<SideSection v-if="unstaged.length" title="Changes" :count="unstaged.length">
			<div v-for="change in unstaged" :key="change.path" class="scm-row">
				<button
					class="tree-row scm-row__file"
					:title="change.path"
					@click="showDiff(change, false)"
				>
