...
const recentJobs = computed(() =>
	[...(jobs.data.value ?? []), ...shell.hostJobs].slice(-40).reverse(),
)
...
<template>
	<section class="dock" :style="{ height: shell.dockHeight + 'px' }" aria-label="Panel">
		...
		<div class="dock__body">
			...
			<template v-else-if="shell.dockTab === 'output'">
				...
				<div v-else class="dock-output">
					<div
						...
						:class="[
							line.tone !== 'info' ? `dock-output__line--${line.tone}` : undefined,
							{ 'dock-output__line--host': line.fromHost },
						]"
					>
						<span class="dock-output__time">{{ line.time }}</span
						><span v-if="line.fromHost" class="dock-output__source">HOST</span>{{ line.text }}
					</div>
				</div>
			</template>
				...
				<div class="term">
					<div ref="scrollback" class="term__scroll">
						<p v-if="!shell.terminal.length" class="dock-empty">
							pw4shell in <code>{{ terminalFolder ?? 'workspace' }}</code> &mdash; type
							<code>help</code> for kernel commands or <code>packwand --help</code>
							for the full CLI.
						</p>
						<div
			...
			<template v-else>
				<p v-if="!recentJobs.length" class="dock-empty">
					No background jobs have run in this session.
				</p>
				<div v-else class="dock-output">
					<div v-for="job in recentJobs" :key="job.id" class="problem-row">
						<AppIcon
							:name="
								job.status === 'failed' ? 'error' : job.status === 'running' ? 'sync' : 'check'
							"
							:size="14"
							class="problem-row__icon"
						/>
