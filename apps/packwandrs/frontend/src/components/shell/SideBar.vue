...
import CollabSidebar from '@/components/shell/CollabSidebar.vue'
...
<template>
	<aside
		...
		:aria-label="
			shell.sidebarMode === 'explorer'
				? 'Explorer'
				: shell.sidebarMode === 'source-control'
					? 'Source Control'
					: shell.sidebarMode === 'collab'
						? 'Live Collaboration'
						: 'Extensions'
		"
	>
		...
		<div v-if="shell.sidebarMode === 'explorer'" class="side-body">
			...
			<SideSection
				...
				title="Variants"
				:count="workbench.selectedProject.manifest.variants?.length ?? 0"
				:open="false"
			>
		...
		<div v-else-if="shell.sidebarMode === 'collab'" class="side-body">
			<CollabSidebar />
		</div>
