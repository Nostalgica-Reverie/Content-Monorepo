...
const sections = computed(() => [
	...
	},
	{
		...
		label: 'Available',
		items: matching(
			extensions.manifests.filter((manifest) => !extensions.isInstalled(manifest.id)),
		),
	},
])
...
<template>
	<div class="extensions-browser">
		...
		<template v-for="section in sections" :key="section.id">
			...
			<p v-if="!section.items.length && !query" class="side-empty">
				{{
					section.id === 'installed'
						...
						: 'Every bundled extension is installed.'
				}}
			</p>
			...
			>
				<button
					class="extension-card__summary"
					:aria-expanded="expanded === manifest.id"
					@click="toggle(manifest.id)"
				>
					...
					<span
						...
						:title="
							extensions.isInstalled(manifest.id)
								? active(manifest)
									? 'Active for this project'
									: 'Installed; inactive for this project'
				...
				<div v-if="expanded === manifest.id" class="extension-card__details">
					...
					<div class="extension-capabilities" aria-label="Declared capabilities">
						<span v-for="capability in manifest.capabilities" :key="capability">{{
							capability
						}}</span>
					</div>
					...
					<button
						v-if="!extensions.isInstalled(manifest.id)"
						class="extension-install"
					...
					<p
						v-if="extensions.isInstalled(manifest.id) && !active(manifest)"
						class="extension-card__hint"
					>
		...
		<p class="extensions-footnote">
			Extensions are installed manually from Packwand's bundled, first-party catalog.
		</p>
	</div>
</template>
...
.extension-card__copy small {
	color: var(--faint);
	font-size: 9.5px;
	font-weight: 550;
}
.extension-card__state {
	display: grid;
	width: 17px;
	height: 17px;
	place-items: center;
...
.extension-card__state--inactive {
