	...
	<article class="panel span-12 theme-workshop">
		<div class="panel-head">
			<div>
				...
				<p class="panel-copy">
					Themes color the complete Packwand shell and editor. Custom themes are safe JSON stored in
		...
		<div class="theme-gallery">
			<button
				...
				type="button"
				class="theme-card"
				:class="{ active: themes.currentId === theme.id }"
				@click="themes.setTheme(theme.id)"
				@dblclick="edit(theme)"
			>
				<span class="theme-card__swatches"
					><i
						v-for="token in ['bg', 'surface', 'accent', 'text']"
						:key="token"
						:style="{ background: theme.colors[token as keyof typeof theme.colors] }"
				/></span>
				<strong>{{ theme.name }}</strong
				><small
					>{{ theme.appearance }} &middot;
					{{ theme.id.startsWith('builtin.') ? 'built in' : 'custom' }}</small
				>
				<span class="theme-card__actions" @click.stop
					><Button variant="quiet" @click="edit(theme)">{{
						theme.id.startsWith('builtin.') ? 'Duplicate' : 'Edit'
					}}</Button></span
				>
			</button>
		</div>
		<p v-for="diagnostic in themes.diagnostics" :key="diagnostic" class="notice">
			{{ diagnostic }}
		</p>
		<div v-if="draft" class="theme-editor">
			<div class="theme-editor__form">
				<label class="field-stack"
					><span>Name</span><input v-model="draft.name" @input="updateSource()"
				/></label>
				<label class="field-stack"
					><span>ID</span><input v-model="draft.id" @input="updateSource(false)"
				/></label>
				<label class="field-stack"
					><span>Appearance</span
					><select v-model="draft.appearance" @change="updateSource()">
				...
				<div class="theme-colors">
					<label v-for="token in themeTokenNames" :key="token"
						><span>{{ token }}</span
						><input
							type="color"
							:value="draft.colors[token]?.slice(0, 7)"
							@input="setColor(token, $event)"
						/><code>{{ draft.colors[token] }}</code></label
					>
				</div>
			</div>
			<div class="theme-editor__json">
				<label class="field-stack"
					...
					><textarea v-model="source" spellcheck="false" @input="updateFromSource" />
				</label>
				<p v-if="parseError" class="error-banner">{{ parseError }}</p>
				<p v-for="message in validation?.errors" :key="message" class="error-banner">
					{{ message }}
				</p>
			...
			<div class="panel-actions theme-editor__actions">
				...
				<Button
					v-if="
						draft.id.startsWith('user.') && themes.themes.some((theme) => theme.id === draft?.id)
					"
					variant="danger"
					@click="remove"
					>Delete</Button
				>
				<Button :busy="saving" :disabled="Boolean(parseError) || !validation?.valid" @click="save"
					>Save theme</Button
				>
			</div>
		</div>
