 ...
 *
 * The handle is the author's display name, which is what both providers key
 * their creator lookups on — Modrinth by username, CurseForge by the name
 * embedded in the mod payload.
 */
function openCreator() {
	if (!shown.value.author) return
	void router.push({
		name: 'creator',
		params: { provider: props.provider, handle: shown.value.author },
	})
}
</script>
	...
	<aside class="project-detail" aria-label="Project details">
		<header class="detail-head">
			...
			<div class="detail-title">
				...
				<small
					>{{ numberFormat.format(shown.downloads) }} downloads<template v-if="shown.author">
						· {{ shown.author }}</template
					></small
				>
			</div>
		...
		<div class="detail-meta">
			...
			<span v-for="loader in shown.loaders.slice(0, 6)" :key="loader" class="chip">{{
				loader
			}}</span>
			<span v-for="version in shown.gameVersions.slice(0, 6)" :key="version" class="chip subtle">{{
				version
			}}</span>
		</div>
