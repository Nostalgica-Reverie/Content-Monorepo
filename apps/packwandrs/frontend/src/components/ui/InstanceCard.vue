...
const subtitle = computed(() => instanceVersionLabel(props.instance.loader, props.instance.gameVersion))
const sourcePath = computed(() => props.instance.source.kind === 'linked' ? props.instance.source.packDir : 'Private standalone pack')
...
const playedLabel = computed(() => props.instance.lastPlayedMs
  ? new Intl.RelativeTimeFormat(undefined, { numeric: 'auto' }).format(
      Math.round((props.instance.lastPlayedMs - Date.now()) / 86_400_000), 'day',
    )
  : 'Never played')
...
<template>
  <article class="instance-card" :class="[`instance-card--${phase}`, { 'instance-card--art': backgroundUrl }]">
    <div class="instance-card__art" :style="backgroundUrl ? { backgroundImage: `url(${backgroundUrl})` } : undefined">
      <div class="instance-card__shade" />
      <div class="instance-card__badges">
        <span class="instance-card__badge"><AppIcon name="mods" :size="11" />{{ instance.loader }}</span>
        <span v-if="playing || starting" class="instance-card__badge instance-card__badge--live"><i />{{ playing ? 'Running' : 'Starting' }}</span>
      </div>
    </div>

    <div class="instance-card__identity">
      <img v-if="iconUrl" :src="iconUrl" class="instance-card__icon" alt="" />
      <div v-else class="instance-card__icon instance-card__icon--placeholder"><AppIcon name="instances" :size="28" /></div>
    ...
    <div class="instance-card__body">
      <div class="instance-card__source" :title="sourcePath"><AppIcon :name="instance.source.kind === 'linked' ? 'target' : 'folder'" :size="12" />{{ sourceName }}</div>
      <div class="instance-status" :class="'instance-status--' + phase"><i class="instance-status-dot" /><span>{{ statusText() }}</span></div>
    ...
    <footer class="instance-card__footer">
      ...
      <Button v-else :busy="modLoading" @click="play"><AppIcon name="play" :size="13" /> Play</Button>
    </footer>
  </article>
</template>
