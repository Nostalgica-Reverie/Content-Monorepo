...
<template>
  <div ref="viewport" class="log-viewer" :style="{ height: `${viewportHeight}px` }" @scroll="scrollTop = ($event.target as HTMLElement).scrollTop">
    <div :style="{ height: `${lines.length * rowHeight}px`, position: 'relative' }">
      <div :style="{ transform: `translateY(${start * rowHeight}px)` }">
        <div v-for="(line, index) in visible" :key="start + index" class="log-viewer__line">{{ line }}</div>
      </div>
    </div>
  </div>
</template>
