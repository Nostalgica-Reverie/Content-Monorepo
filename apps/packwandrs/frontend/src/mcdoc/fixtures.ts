...
const literal = (value: string): McdocType => ({
	kind: 'literal',
	...
	fields: [
		{
			kind: 'pair',
			key: 'function',
			type: id('minecraft:loot_function'),
			desc: 'Which function to apply.',
		},
		{
			kind: 'spread',
			type: {
...
const predicate: McdocType = {
	kind: 'union',
	members: [lootCondition, { kind: 'list', item: lootCondition }],
}
				...
				fields: [
					{
						kind: 'pair',
						key: 'chance',
						type: { kind: 'float', valueRange: { kind: 0, min: 0, max: 1 } as never },
					},
				],
			},
