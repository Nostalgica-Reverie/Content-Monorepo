...
function removeItem(index: number) {
  emit('update', items.value.filter((_, at) => at !== index))
}
...
const isInteger = computed(() =>
  ['byte', 'short', 'int', 'long'].includes(simplified.value.kind),
)
...
<template>
  <div class="mcdoc-field">
    ...
    <p v-if="depth >= MAX_RENDER_DEPTH" class="mcdoc-field__note">
      Nested too deeply to render.
    </p>
    ...
    <div v-else-if="simplified.kind === 'list'" class="mcdoc-list">
      <div v-for="(_, index) in items" :key="index" class="mcdoc-list__row">
        ...
        <div class="mcdoc-list__actions">
          <button class="mcdoc-btn mcdoc-btn--quiet" type="button" title="Move up" @click="moveItem(index, -1)">↑</button>
          <button class="mcdoc-btn mcdoc-btn--quiet" type="button" title="Move down" @click="moveItem(index, 1)">↓</button>
          <button class="mcdoc-btn mcdoc-btn--quiet" type="button" title="Remove" @click="removeItem(index)">×</button>
        </div>
      </div>
    ...
    <label v-else-if="simplified.kind === 'boolean'" class="mcdoc-check">
      <input type="checkbox" :checked="value === true" @change="emit('update', ($event.target as HTMLInputElement).checked)" />
