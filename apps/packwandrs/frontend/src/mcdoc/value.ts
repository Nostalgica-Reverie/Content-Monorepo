...
import { childPath, isRecord, rootPath, simplifyType, structFields, type ValuePath } from './simplify'
...
export function defaultValue(type: McdocType, source: SchemaSource, depth = 0): unknown {
  ...
  switch (simplified.kind) {
    ...
    case 'long':
      return 0
    case 'literal':
      return simplified.value.kind === 'long' ? Number(simplified.value.value) : simplified.value.value
    case 'enum':
      return simplified.values[0]?.value
    case 'union':
      return simplified.members.length ? defaultValue(simplified.members[0], source, depth + 1) : undefined
    default:
      return undefined
  }
}
...
function matchScore(type: McdocType, path: ValuePath, source: SchemaSource, depth = 0): number {
  ...
  switch (simplified.kind) {
    case 'literal': {
      const literal = simplified.value.kind === 'long' ? Number(simplified.value.value) : simplified.value.value
    ...
    case 'struct': {
      ...
      for (const field of fields) {
        ...
        if (present) {
          ...
          score += matchScore(field.type, childPath(path, field.key), source, depth + 1) >= 100 ? 100 : 0
 ...
 */
export function attributeString(value: AttributeValue | undefined, treeKey = 'registry'): string | undefined {
