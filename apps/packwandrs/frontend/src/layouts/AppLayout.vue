...
import {
	diagnosticsContentLint,
	diagnosticsLint,
	diagnosticsPreflight,
	diagnosticsValidate,
} from '@/helpers/invoke/diagnostics'
...
onMounted(async () => {
		...
		stopKernelTrace = await onKernelTrace((record) => {
			const origin =
				record.platformCode === null
					? record.origin
					: `${record.origin} code ${record.platformCode}`
	...
	try {
		if (!collab.isGuest) await workbench.refresh()
		shell.appendOutput(
			`Indexed ${workbench.projects.length} projects, ${workbench.packs.length} pack targets.`,
		)
	} catch (error) {
...
watch(
	...
	(name) => {
		...
		if (item)
			shell.openTab({ name: item.name, path: item.path, label: item.label, icon: item.icon })
	},
	{ immediate: true },
)
...
const commands = computed<PaletteCommand[]>(() => [
	...
	...(workbench.selectedPack
		...
		: []),
	...(collab.role === null && workbench.selectedPack
		? [
				{
					id: 'collab:start',
					...
					icon: 'users',
					run: async () => {
						await collab.host(workbench.selectedPack!.id)
						shell.showSidebar('collab')
					},
				},
			]
		: []),
...
<template>
	...
	>
		<header class="titlebar">
			...
			<span class="titlebar-workspace" :title="workspace.path ?? ''">{{
		...
		<div class="editor-area">
			...
			<div class="editor-scroll">
				...
				<section v-if="showCommandRow" class="toolbar command-toolbar">
					<Button
						variant="secondary"
						:busy="busy === 'Validate workspace'"
						@click="run('Validate workspace', diagnosticsValidate)"
					>
						Validate
					</Button>
					<Button
						variant="quiet"
						:busy="busy === 'Preflight'"
						@click="run('Preflight', diagnosticsPreflight)"
					>
						Preflight
					</Button>
					<Button variant="quiet" :busy="busy === 'Lint'" @click="run('Lint', diagnosticsLint)"
						>Lint</Button
					>
					<Button
						variant="quiet"
						:busy="busy === 'Dry sync'"
						@click="run('Dry sync', workspaceSyncPreview, syncSummary)"
					>
						Dry sync
					</Button>
				</section>
