...
const totalIndexedFiles = computed(() =>
	workbench.projectPacks.reduce((total, pack) => total + pack.indexedFiles, 0),
)
const totalMetadataFiles = computed(() =>
	workbench.projectPacks.reduce((total, pack) => total + pack.metadataFiles, 0),
)
const projectLoaders = computed(() =>
	[...new Set(workbench.projectPacks.flatMap((pack) => pack.loaders))].sort(),
)
...
async function run(name: string, work: () => Promise<unknown>) {
	...
	try {
		...
		const issues =
			result && typeof result === 'object' && Array.isArray((result as ValidationReport).issues)
		...
		shell.appendOutput(
			`${name}: ${count ? `${count} issue(s).` : 'completed.'}`,
			count ? 'error' : 'success',
		)
		toasts.push(
			name,
			count ? `${count} issues found.` : 'Completed successfully.',
...
async function launchSelectedPack() {
	...
	try {
		...
		let instance = instances.items.find(
			(item) =>
				item.source.kind === 'linked' &&
				item.source.packDir.replaceAll('\\', '/').toLowerCase() ===
					pack.path.replaceAll('\\', '/').toLowerCase(),
		)
		if (!instance) {
			if (!confirm('This pack has no linked instance. Create an isolated test instance now?'))
				return
...
const actions = [
	{
		label: 'Validate',
		icon: 'shield',
		hint: 'Manifests, roles, variants, releases',
		run: () => run('Validate workspace', diagnosticsValidate),
	},
	{
		label: 'Preflight',
		icon: 'check',
		hint: 'Composite release gate',
		run: () => run('Preflight', diagnosticsPreflight),
	},
	{
		label: 'Dry sync',
		icon: 'sync',
		hint: 'Preview base-to-consumer changes',
		run: () => run('Sync preview', workspaceSyncPreview),
	},
]
</script>

<template>
	<!-- A single root element, not a Fragment: AppLayout wraps every routed page
       in a `<Transition mode="out-in">`, which needs one real element to
       attach enter/leave hooks to. A Fragment root (this used to be two
       sibling `<section>`s plus a trailing `<Modal>`) makes the leave phase
       never resolve, which wedges every navigation after the first — see
       the sibling pages for the same shape of fix. -->
	<div>
	...
	<section v-else class="overview">
		...
		<div class="overview__main">
			...
			<div v-if="workbench.selectedPack" class="selected-target">
				...
				<dl>
					<div>
						<dt>Minecraft</dt>
						<dd>{{ workbench.selectedPack.minecraftVersion || 'unset' }}</dd>
					</div>
					<div>
						<dt>Loader</dt>
						<dd>{{ workbench.selectedPack.loaders.join(', ') || 'unset' }}</dd>
					</div>
					<div>
						<dt>Version</dt>
						<dd>{{ workbench.selectedPack.version || 'unset' }}</dd>
					</div>
				</dl>
			...
			<div class="action-row panel-bottom-actions">
				<Button :busy="busy === 'Apply sync'" @click="run('Apply sync', workspaceSync)"
					>Apply workspace sync</Button
				>
		...
		<aside class="overview__aside">
			...
			<dl class="facts">
				...
				<div>
					<dt>Version</dt>
					<dd>{{ manifest?.version || 'unset' }}</dd>
				</div>
				<div>
					<dt>Lifecycle</dt>
					<dd>{{ manifest?.lifecycle || 'active' }}</dd>
				</div>
				<div>
					<dt>Minecraft</dt>
					<dd>{{ manifest?.mc_version || 'per variant' }}</dd>
				</div>
				<div>
					<dt>Loader</dt>
					<dd>{{ manifest?.loader || 'per variant' }}</dd>
				</div>
				<div>
					<dt>Targets</dt>
					<dd>{{ workbench.projectPacks.length }}</dd>
				</div>
				<div>
					<dt>Variants</dt>
					<dd>{{ manifest?.variants?.length || 0 }}</dd>
				</div>
			</dl>
	...
	<Modal :open="createOpen" title="Create project" @close="createOpen = false">
		<form class="form-grid" @submit.prevent="createProject">
			...
			<label
				><span>Project ID</span><input v-model="form.id" required placeholder="my-project"
			/></label>
			<label
				><span>Display name</span><input v-model="form.name" placeholder="Defaults to ID"
			/></label>
			...
			<label
				><span>Variants</span
				><input v-model="form.variants" placeholder="1.21.1-fabric, 1.21.4-fabric"
			/></label>
