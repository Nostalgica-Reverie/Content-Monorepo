...
async function run() {
  ...
  failure.value = null
  try {
    const page = await providerBrowse(provider.value, {
...
watch(() => workbench.selectedPackId, () => schedule())
    ...
    <article class="panel span-12">
      ...
      <div class="browse-controls">
        ...
        <label class="browse-search">
          <span class="sr-only">Search {{ provider === 'modrinth' ? 'Modrinth' : 'CurseForge' }}</span>
      ...
      <ul v-if="results.length" class="result-list" role="list">
        ...
        >
          <img v-if="project.iconUrl" :src="project.iconUrl" alt="" class="result-icon" loading="lazy" />
          ...
          <div class="result-body">
            ...
            <div class="result-meta">
              ...
              <span v-for="loader in project.loaders.slice(0, 4)" :key="loader" class="chip">{{ loader }}</span>
            </div>
          </div>
      ...
      <div v-if="results.length" class="action-row panel-bottom-actions">
        ...
        <span class="page-indicator">
          {{ offset + 1 }}–{{ offset + results.length }}<template v-if="total"> of {{ downloads(total) }}</template>
        </span>
        <Button variant="quiet" :disabled="!canPageForward" @click="page(PAGE_SIZE)">
          Next
        </Button>
      </div>
    </article>
...
<style scoped>
.browse-controls { display: flex; gap: 0.75rem; align-items: center; flex-wrap: wrap; }
.browse-search { flex: 1; min-width: 14rem; }
.browse-search input { width: 100%; }

.provider-switch { display: inline-flex; gap: 2px; padding: 2px; background: var(--surface-2); border-radius: 0.4rem; }
.provider-tab {
...
.provider-tab.is-active { color: var(--text-strong); background: var(--surface); }

.browse-split { display: grid; grid-template-columns: 1fr; gap: 0.75rem; }
/* The detail panel only earns a column when there is room for both. */
@media (min-width: 62rem) {
  .browse-split:has(.project-detail) { grid-template-columns: minmax(0, 1fr) minmax(20rem, 26rem); }
}
.project-detail { max-height: 34rem; border-radius: 0.5rem; border: 1px solid var(--line); }

.result-list { display: flex; flex-direction: column; gap: 0.5rem; margin: 0.75rem 0 0; padding: 0; list-style: none; }
.result {
  display: flex;
  gap: 0.75rem;
  padding: 0.6rem;
...
.result.is-open { border-color: var(--accent); }

.result-icon { width: 3rem; height: 3rem; object-fit: cover; border-radius: 0.375rem; }
.result-icon.placeholder { background: var(--surface-2); }
.result-body { flex: 1; min-width: 0; }
.result-head { display: flex; align-items: baseline; gap: 0.5rem; justify-content: space-between; }
.result-head strong { color: var(--text-strong); }
.result-head small { color: var(--faint); font-size: 0.7rem; white-space: nowrap; }
.result-summary {
...
.result-meta { display: flex; flex-wrap: wrap; gap: 0.4rem; color: var(--faint); font-size: 0.7rem; }
.chip { padding: 0.05rem 0.35rem; background: var(--surface-2); border-radius: 999px; }

.result-actions { display: flex; flex-direction: column; gap: 0.3rem; align-items: stretch; }
.result-link {
  color: var(--accent-2);
  font-size: 0.7rem;
  text-align: center;
...
.page-indicator { color: var(--faint); font-size: 0.75rem; }
