...
export function bit_array_pad_to_bytes(bit_array) {
	...
	if (finalByte === correctFinalByte) {
		return new BitArray(bit_array.rawBuffer, bit_array.byteSize * 8, bit_array.bitOffset)
	}
...
export function base64_encode(bit_array, padding) {
	...
	for (let i = 0, j = 0; j < m; i += 4, j += 3) {
		const y =
			(bit_array.byteAt(j) << 16) + (bit_array.byteAt(j + 1) << 8) + (bit_array.byteAt(j + 2) | 0)
...
export function bit_array_starts_with(bits, prefix) {
	...
	if (prefix.bitSize % 8 !== 0) {
		...
		if (bits.byteAt(byteCount) >> unusedBitsCount !== prefix.byteAt(byteCount) >> unusedBitsCount) {
			return false
		}
	}
