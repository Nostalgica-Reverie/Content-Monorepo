...
const totalCount = computed(() => groups.value.reduce((n, g) => n + g.mods.length, 0))
...
<template>
	<div class="modlist">
		...
		<div v-else-if="error" class="modlist-status modlist-error">Could not load the mod list.</div>
		<template v-else>
