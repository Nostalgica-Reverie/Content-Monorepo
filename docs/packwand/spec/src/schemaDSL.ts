...
declare global {
	namespace Reflect {
		export function getMetadata<T>(metadataKey: typeof objectProperties, target: T): ObjectProperties
...
export abstract class property {
	...
	static ref(description?: string) {
		return _updateMetadata({
			description,
		}) as <T extends Record<K, SchemaGenerator>, K extends string | number | symbol>(
			target: T,
			propertyKey: K,
		) => void
