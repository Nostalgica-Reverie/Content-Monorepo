...
const markerPath = join(here, '..', '..', 'node_modules', '.cache', 'packwand-commands-tree')

function packwandTreeState(): string | null {
	try {
		const dirty = execFileSync('git', ['status', '--porcelain', '--', packwandSrc], {
			encoding: 'utf-8',
			cwd: here,
		}).trim()
...
const temporaryCommandsDir = await mkdtemp(join(commandsParent, '.packwand-commands-'))
let committed = false

try {
	execFileSync(bin, [...binArgs, 'utils', 'markdown', '--dir', temporaryCommandsDir], {
		stdio: 'inherit',
	})
	...
	console.log('Regenerated and sanitized the Packwand CLI reference.')
} finally {
	if (!committed) {
