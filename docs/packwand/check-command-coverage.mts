...
if (!Array.isArray(commandPaths)) {
  console.error("`packwand utils commands --json` did not return a JSON array as expected; got:", commandPaths);
  process.exit(1);
}
...
if (missing.length > 0) {
  console.error(`CLI-reference coverage: ${missing.length} registered command(s) have no generated page:`);
  for (const name of missing) console.error(`  - ${name}`);
}
if (orphaned.length > 0) {
  console.warn(`CLI-reference coverage: ${orphaned.length} stale page(s) no longer correspond to a registered command:`);
  for (const name of orphaned) console.warn(`  - ${name}`);
}

if (missing.length > 0) process.exit(1);
console.log(`CLI-reference coverage OK: ${commandPaths.length} command(s), all with a generated handbook page.`);
