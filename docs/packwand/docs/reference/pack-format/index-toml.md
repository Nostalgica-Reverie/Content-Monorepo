...
**String, required.** The default [hash format](/reference/pack-format/pack-toml#hash-formats) for every file in the index. If missing, consumers assume `sha512`.
...
Files matching the pack's [`.packwizignore`](/reference/pack-format/packwizignore) rules (or the built-in defaults) are never added to the index. The pack file, the index itself, and `.packwizignore` are always excluded.
