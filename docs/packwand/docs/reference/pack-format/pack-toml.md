...
**Table, optional.** Tool configuration read at load time; see [Additional options](/reference/additional-options). Keys include `acceptable-game-versions`, `acceptable-game-loaders`, `meta-folder`, `meta-folder-base`, `no-internal-hashes`, and `datapack-folder`.
