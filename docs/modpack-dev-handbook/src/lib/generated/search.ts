...
export const searchPages: SearchPage[] = [
  ...
  },
  {
    "title": "manifest.json",
    "description": "",
    "content": "manifest.json The packwand manifest is the root metadata file for a publishable project directory. It stores the project's identity, loader/version matrix, publishing identifiers, role, lifecycle, and automation settings. packwand reads and writes manifest.json in each pack directory. Commands such as packwand new, packwand validate, packwand publish, packwand automation, and the workspace operations all treat it as the pack's source of truth. Required fields id Unique pack identifier, usually the directory name name Human readable pack name type Project kind: mod, modpack, datapack, or resourcepack role Pack role, usually none, base, or a consumer/base mapping object Common fields loader Primary loader for the pack mc version Primary Minecraft version for the pack variants Optional variant list for multi version packs version Pack release version release type Release channel label used by publish workflows description Short pack description $schema Optional schema URL for editor tooling modrinth id, curseforge id, github id, gitea id, gitlab id External publishing identifiers shared assets Shared asset path used by base/consumer pack layouts lifecycle Pack maintenance state: active, maintenance, archived, or eol Variants Each entry in variants is an object with: id Optional variant identifier name Optional display name mc version Minecraft version for that variant loader Optional loader override for that variant version Optional variant specific pack version gradle project Stonecutter Gradle subproject to build. Required for every mod variant. Mods must use multi variant manifests, declare a loader and gradle project for every variant, and use role: \"none\". Pack to pack role and automation.sync variants settings do not apply to mod source projects. Role role is deliberately flexible so the pack can describe both simple and workspace aware setups. \"none\" is the default for ordinary packs \"base\" marks a performance base pack { \"performance base\": { \"pack\": \"...\", \"mappings\": [...] } } marks a consumer pack that syncs content from a base pack Automation automation is optional. It controls unattended update and release behavior. auto update enables or disables automatic update flows server promo marks a pack for server promotion workflows sync exclude lists paths to skip during workspace sync freeze maps subdirs to frozen mod slugs that should not update full auto.enabled opts into the end to end packwand automation run pipeline full auto.tests is an optional list of shell commands run before the manifest version bump Example A Stonecutter mod maps each release directly to an existing Gradle subproject:",
...
export const searchSections: SearchSection[] = [
  ...
  },
  {
    ...
    "sectionTitle": "Example",
    "content": "A Stonecutter mod maps each release directly to an existing Gradle subproject:",
  ...
  },
  {
    ...
    "content": "id Unique pack identifier, usually the directory name name Human readable pack name type Project kind: mod, modpack, datapack, or resourcepack role Pack role, usually none, base, or a consumer/base mapping object",
  ...
  },
  {
    ...
    "content": "Each entry in variants is an object with: id Optional variant identifier name Optional display name mc version Minecraft version for that variant loader Optional loader override for that variant version Optional variant specific pack version gradle project Stonecutter Gradle subproject to build. Required for every mod variant. Mods must use multi variant manifests, declare a loader and gradle project for every variant, and use role: \"none\". Pack to pack role and automation.sync variants settings do not apply to mod source projects.",
  ...
  },
  {
    ...
    "content": "The About menu shows bundled third party licenses from src/licenses.html. Regenerate it after dependency changes with just gen licenses (or the commands in the README).",
