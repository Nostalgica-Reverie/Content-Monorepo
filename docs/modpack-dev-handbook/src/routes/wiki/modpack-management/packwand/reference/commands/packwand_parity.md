...
Report content drift between a pack's Modrinth (-mr) and CurseForge (-cf) variant subdirs (no args: all modpacks)

```
packwand parity [pack-dir...] [flags]
```
...
```
  -h, --help     help for parity
      --json     Output the parity report as JSON
      --strict   Exit 1 when any variant pair has drifted
```
