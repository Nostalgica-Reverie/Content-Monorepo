...
function merge(consumer: Changelog, base: Changelog | undefined, baseLabel: string): string {
    ...
    if (base) {
        ...
        if (consumerUsesSections && baseSections.length > 0) {
            ...
            const body = baseSections.length
                ? baseSections.flatMap((section) => [`**${section.title}**`, ...trimBlank(section.body), ''])
    ...
    const ordered = sections
        ...
        .sort((left, right) => rank(left.section.title) - rank(right.section.title) || left.index - right.index)
...
if (!manifestArg) {
    console.error('usage: bun run.ts build-release-notes <path/to/manifest.json> [output-file] [--dry-run]');
    process.exit(1);
}
...
if (baseId) {
    ...
    if (!baseRoot) {
        console.error(`warning: performance base ${JSON.stringify(baseId)} was not found; notes cover this pack only`);
    } else {
        ...
        if (fs.existsSync(baseChangelog)) {
            ...
            console.error(`warning: performance base ${baseId} has no changelog.md; notes cover this pack only`);
        }
    }
}
