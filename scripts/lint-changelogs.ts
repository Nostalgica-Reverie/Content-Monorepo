...
function lintProject(projectRoot: string, issues: Issue[]): void {
    ...
    if (!fs.existsSync(changelogPath)) {
        issues.push({ severity: 'error', project: name, path: relative, message: 'changelog.md is missing' });
        return;
    }
    ...
    if (!heading) {
        issues.push({ severity: 'error', project: name, path: relative, message: 'changelog has no heading' });
    ...
    const ranks = known.map((section) => SECTION_ORDER.indexOf(section as (typeof SECTION_ORDER)[number]));
...
if (asJson) {
    ...
    for (const issue of issues) {
        console.log(`${issue.severity === 'error' ? 'ERROR' : 'warn '}  ${issue.path}: ${issue.message}`);
    }
