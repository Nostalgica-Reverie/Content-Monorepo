...
function collectModrinthIds(): Map<string, string> {
	...
	try {
		entries = JSON.parse(
			execFileSync(packwand, ['packs', 'list', '--json'], { encoding: 'utf-8' }),
		) as PackEntry[]
	} catch (error) {
...
let totalFo = 0
console.log(
	`${'pack'.padEnd(nameW)}  ${'downloads'.padStart(10)}  ${'followers'.padStart(9)}  updated`,
)
...
console.log(
	`${'total'.padEnd(nameW)}  ${totalDl.toLocaleString('en-US').padStart(10)}  ${totalFo.toLocaleString('en-US').padStart(9)}`,
)
