...
public class ReloadExecutor {
            ...
            ForkJoinWorkerThread forkJoinWorkerThread = new ForkJoinWorkerThread(forkJoinPool) {
                protected void onTermination(Throwable throwOnTermination) {
                    if (throwOnTermination != null) {
                        Modernica.LOGGER.warn("{} died", this.getName(), throwOnTermination);
                    } else {
                        Modernica.LOGGER.debug("{} shutdown", this.getName());
                    }
    ...
    private static void handleException(Thread thread, Throwable throwable) {
        ...
        Modernica.LOGGER.error(String.format("Caught exception in thread %s", thread), throwable);
    }
}
