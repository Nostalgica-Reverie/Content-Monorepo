package net.nostalgica.modernica.common.mixin.perf.dynamic_sounds;
...
import net.nostalgica.modernica.dynresources.DynamicSoundHelpers;
...
public abstract class SoundBufferLibraryMixin {
    ...
    private static final boolean debugDynamicSoundLoading = Boolean.getBoolean("modernica.debugDynamicSoundLoading");
    ...
    private <K extends Identifier, V extends CompletableFuture<SoundBuffer>> void onSoundRemoval(RemovalNotification<K, V> notification) {
        ...
        if(k == null)
            return;
        Modernica.LOGGER.warn("Evicted sound {}", k);
    }
}
