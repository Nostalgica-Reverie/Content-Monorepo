package net.nostalgica.modernica.dfu;
    ...
    private static void tryAdd(List<Map<?, ?>> list, String className, String fieldName) {
        try {
            ...
            if (Map.class.isAssignableFrom(field.getType())) {
                ...
                Modernica.LOGGER.error("Field {} on class {} is not a map", field, className);
            }
        } catch (ReflectiveOperationException e) {
            Modernica.LOGGER.error("Error tracking DFU field {} on class {}", fieldName, className, e);
        }
    }
