package net.nostalgica.modernica.textures;
            ...
            while(true) {
                ...
                try (STBRPNode.Buffer nodes = STBRPNode.malloc(longestWidth + 10)) {
                    ...
                    if(numTries >= 4) {
                        ...
                        Modernica.LOGGER.error("Stitcher ran out of space with target atlas size " + longestWidth + "x" + longestHeight + ":");
                        for(Stitcher.Holder<T> h : holders) {
                            Modernica.LOGGER.error(" - " + h.entry().name() + ", " + h.width() + "x" + h.height());
                        }
                        throw e;
                    } else {
