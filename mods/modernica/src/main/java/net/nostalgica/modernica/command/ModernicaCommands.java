package net.nostalgica.modernica.command;
...
import net.nostalgica.modernica.duck.IProfilingServerFunctionManager;

import static net.minecraft.commands.Commands.literal;

public class ModernicaCommands {
    public static void register(CommandDispatcher<CommandSourceStack> dispatcher) {
        dispatcher.register(literal("modernica")
                .then(literal("mcfunctions").requires(source -> source.permissions().hasPermission(Permissions.COMMANDS_ADMIN))
                        .executes(context -> {
                            ...
                            if (level.getServer().getFunctions() instanceof IProfilingServerFunctionManager profiler) {
                                ...
                                context.getSource().sendFailure(Component.literal("Modernica mcfunction profiling is not enabled on this server."));
                                return 0;
                            }
                        }))
        );
