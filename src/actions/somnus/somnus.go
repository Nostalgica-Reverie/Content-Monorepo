...
func main() {
	if len(os.Args) < 2 {
		fail("usage: somnus <init|bump|export|sync|modlist|test|lint> [args]")
	}
	...
	case "lint":
		cmdLint(os.Args[2:])
	default:
		fail(fmt.Sprintf("unknown verb %q (expected init, bump, export, sync, modlist, test, or lint)", os.Args[1]))
	}
}
