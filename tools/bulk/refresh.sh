...
if [ "${#TARGETS[@]}" -eq 0 ]; then
  echo "no pack subdirs to refresh."
  exit 0
fi
...
printf '%s\n' "${TARGETS[@]}" | MODPACKS_DIR="$MODPACKS_DIR" xargs -P "$MAX_CONCURRENT" -I{} bash -c '
  ...
  if (cd "$dir" && packwiz refresh) >"/tmp/refresh-$$.log" 2>&1; then
    echo "ok: $label"
    rm -f "/tmp/refresh-$$.log"
  else
    echo "::error::FAIL $label" >&2
    sed "s/^/  /" "/tmp/refresh-$$.log" >&2
    rm -f "/tmp/refresh-$$.log"
    exit 1
  fi
' _ {} || FAILURES=$?
...
if [ "$FAILURES" -ne 0 ]; then
  echo "::error::one or more refreshes failed" >&2
  exit 1
fi

echo "all refreshes finished successfully."
